...
function createCompressor(inStr) {
    ...
    function findLongestMatch() {
        ...
        // A length of 48 is the maximum. See encodeLengthAndDistance().
        while (betterHit && length < 48) {
            distance = betterHit;
            length += 1;
            betterHit = findClosestMatch(length);
        }
    ...
    function encodeLengthAndDistance(bestMatch) {
        ...
        debug("distance %d high %d low %d length %d %j", distance + 1, high, low, 3, inStr.slice(currentByte, currentByte + length));
